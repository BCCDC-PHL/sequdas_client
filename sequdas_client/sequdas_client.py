...
######################################################################
#                                                                    #
# Sequences Upload and Archiving System (SeqUDAS)                    #
#                                                                    #
# Jun Duan                                                           #
...
from sequdas_client.Lib.core import *
from sequdas_client.Lib.check import *
from sequdas_client.Lib.status_log import *
from sequdas_client.Lib.sample_sheet import *
from sequdas_client.Lib.status_db import *
import paramiko
import logging
...
def main():
    ...
    # Local setting
    run_dir_lists=s_config['sequencer']['run_dirs_analysis']
    ...
    ssh_private_key=s_config['local']['ssh_private_key']
    ...
    # Remote setting
    data_server_host= s_config['server']['server_ssh_host']
    data_server_user= s_config['server']['server_ssh_user']
    data_repository=s_config['server']['server_data_dir']
    data_repository=check_path_with_slash(data_repository)
    server_result_dir=s_config['server']['server_result_dir']
    server_result_dir=check_path_with_slash(server_result_dir)
    check_server_folder_para(data_server_host,data_server_user,ssh_private_key, data_repository) 
    print ("Status_1: Passed (The remote server can be connected)")
    data_dir=data_repository+machine
    check_server_folder_para(data_server_host,data_server_user,ssh_private_key, data_dir)
    server_log_dir=data_dir
    now = datetime.datetime.now()
    data_dir =data_dir+"/"+"DATA_"+str(now.year)
    check_server_folder_para(data_server_host,data_server_user,ssh_private_key, data_dir)
    data_server_host_full=data_server_user+"@"+data_server_host
    sequdas_server_dir= check_path_with_slash(s_config['server']['server_sequdas_dir'])
    # Email setting
    ...
    # Make sure local time is the same to NTP server
    if log_details is True:
    ...
    if check_time() is False:
        ...
        if send_email_switch is True:
            send_email(gmail_user,gmail_pass,machine,email_list,"system time error","","")
        sys.exit()
    else:
        if log_details is True:
            logger.info("Check1: The time is correctly set on the client PC\n")
        print ("Status_2: Passed (The time is correctly set on the local PC)")
    #Check the connection with MySQL server
    if checkMySQLdb() is False:
        if log_details is True:
            logger.error("Check3 The MySQL connection cannot be established\n")
        if send_email_switch is True:
            send_email(gmail_user,gmail_pass,machine,email_list,"MySQL connection error","","")
        sys.exit()
    else:
        logger.info("Status_3: Passed (The MySQL connection can be established)\n")
                    
        print ("Status_3: Passed (The MySQL connection can be established)")    
    ...
    for backup_dir in run_dir_lists:
        ...
        if(len(run_with_error_list)>0):
            ...
            for run_with_error in run_with_error_list:            
                ...
                if send_email_switch is True:
                    send_email(gmail_user,gmail_pass,machine,email_list,"Run failed",run_with_error_name,"")
                    if log_details is True:
        ...
        if(len(run_copy_list)>0):
            for run_handle in run_copy_list:                
                ...
                if send_email_switch is True:
                    send_email(gmail_user,gmail_pass,machine,email_list,"Starting",run_name,"")
                ...
                if rsyncproc == 0:
                    ...
                    status_result=md5_compare(data_server_host_full,run_handle,current_ID,data_dir_server,logfile_dir)
                    if status_result=="Status_OK":
                        ...
                        try:
                            ...
                            ssh.connect( hostname = data_server_host, username = data_server_user, pkey = ssh_key )
                            if send_email_switch is True:
                                email_swith=" -e"
                            else:
                                email_swith=""
                            if uploader==1:
                                ...
                                ssh.exec_command("nohup  python "+sequdas_server_dir+"sequdas_server.py -i "+run_path_on_server+" -o"+server_result_dir+" -u "+current_ID+" -c -s 1 -n -t"+email_swith+" >>1.txt 2>&1 &")
                            else:
    ...
    if len(old_file_list)>0:
        ...
        if send_email_switch is True:
            ...
            send_email(gmail_user,gmail_pass,machine,email_list_admin,removal_title,old_file_str,"")
        if log_details is True:	 	
